...
def calc_quick(
    ...
    if not _has_positive(coins_qty):
        return QuickCalc(None, None, None, None, None, None, None)
    ...
    withdraw_amount_rub = base_rub
    fee_rub = _calc_withdraw_fee(
        withdraw_amount_rub,
    ...
    if (
        not _has_positive(settings.withdraw_rate_rub_per_usdt)
        or withdraw_amount_rub is None
        or fee_rub is None
    ):
        return QuickCalc(
            fp_payout_rub_me,
...
def _calc_withdraw_fee(
    ...
    if me_rub is None or withdraw_fee_pct is None or withdraw_fee_min_rub is None:
        return None
    return max(me_rub * withdraw_fee_pct, withdraw_fee_min_rub)
